...
module.exports = {
    ...
    run: async (client, message, args, { GuildDB }) => {
        if (!message.member.voice.channel) return client.sendTime(message.channel, "❌ | **Bạn không ở trong bất kì phòng nào cả!**");
        ...
        if (!SearchString) return client.sendTime(message.channel, `**hãy nhập - **\`${GuildDB.prefix}play [song]\``);
        let CheckNode = client.Manager.nodes.get(client.config.Lavalink.id);
        let Searching = await message.channel.send(":mag_right: đang tìm nhạc...");
       ...
       return client.sendTime(message.channel,"❌ | **Lavalink node not connected**");
        ...
        if (!player) return client.sendTime(message.channel, "❌ | **Bây giờ không có bài nào đang chạy...**");
        ...
        try {
            if (SearchString.match(client.Lavasfy.spotifyPattern)) {
                ...
                if (Searched.loadType === "PLAYLIST_LOADED") {
                    ...
                    SongAddedEmbed.setAuthor(`Thêm vào khu nhạc`, client.config.IconURL);
                    ...
                    return client.sendTime(message.channel, "**Không  tìm thấy nhạc - **" + SearchString);
                }
            } else {
                let Searched = await player.search(SearchString, message.author);
                if (!player) return client.sendTime(message.channel, "❌ | **Bây giờ không có bài nào đang chạy...**");

                if (Searched.loadType === "NO_MATCHES") return client.sendTime(message.channel, "**Không tìm thấy nhạc - **" + SearchString);
            ...
            console.log(e);
            return client.sendTime(message.channel, "**Không tìm thấy bài - **" + SearchString);
        }
    },
        ...
        options: [
            {
                ...
                type: 3,
                required: true,
                description: "Chơi nhạc trong phòng của bạn",
            },
        ],
        /**
         *
            ...
            if (search.match(client.Lavasfy.spotifyPattern)) {
                ...
                try {
                    ...
                    if (res.loadType === "LOAD_FAILED") {
                        if (!player.queue.current) player.destroy();
                        return client.sendError(interaction, `:x: | **Có lỗi khi đang tìm kiếm**`);
                    }
                } catch (err) {
                    return client.sendError(interaction, `*Có lỗi khi đang tìm kiếm: ${err.message}`);
                }
                switch (res.loadType) {
                    case "NO_MATCHES":
                        if (!player.queue.current) player.destroy();
